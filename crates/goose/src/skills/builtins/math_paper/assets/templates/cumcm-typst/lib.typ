// 定理环境 `thmbox` 由同目录的 theorems.typ 提供本地实现。
// 原模板从 `@preview/ctheorems:1.1.3` 导入，那会让编译必须联网（首次编译需下载包）；
// 离线环境下整篇论文都无法编译，因此改为本地实现，功能等价。
...
// 定理环境
//
// 注：原模板此处为 `thmbox.with(base: none, ...)`，依赖 ctheorems 的 base 参数。
// 本地实现（theorems.typ）按显示名生成环境，不需要 base，故已省略该参数。
