...
function computeStats(sessions: SessionListItem[]): Stats {
  ...
  for (const session of sessions) {
    ...
    if (session.workingDir) {
      projectCounts.set(session.workingDir, (projectCounts.get(session.workingDir) ?? 0) + 1);
    }
  ...
  return {
    ...
    prompts,
    projects: [...projectCounts.entries()].sort((a, b) => b[1] - a[1]),
    ...
    <>
      <div className="w-full">
        <header className="flex items-start gap-3">
          <p className="flex-1 text-sm text-text-secondary">{intl.formatMessage(i18n.subtitle)}</p>
        ...
        {loading && !stats && (
          <p className="mt-8 text-sm text-text-secondary">{intl.formatMessage(i18n.loading)}</p>
        )}

        {error && <p className="mt-8 text-sm text-text-danger">{intl.formatMessage(i18n.error)}</p>}
          ...
          <>
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
              ...
              <RankList
                ...
                entries={stats.projects.slice(0, 8)}
                emptyLabel={intl.formatMessage(i18n.none)}
                hideCount
              />
              <RankList
            ...
            <section className="mt-4 rounded-xl border border-border-secondary p-4">
              ...
              {stats.models.length === 0 ? (
                <p className="mt-2 text-xs text-text-tertiary">{intl.formatMessage(i18n.none)}</p>
              ) : (
            ...
            <p className="mt-4 text-xs text-text-tertiary">{intl.formatMessage(i18n.tokensNote)}</p>
          </>
        )}
      </div>
    </>
