...
export function setupAutoUpdater(tray?: Tray) {
  ...
  autoUpdater.on('update-downloaded', (info: UpdateInfo) => {
    ...
    const notification = new Notification({
      title: 'Update Ready',
      body: `Version ${info.version} will be installed when you quit ModelForge. Click to install now.`,
    });
    notification.show();
...
function updateTrayIcon(hasUpdate: boolean) {
  ...
  if (hasUpdate) {
    ...
    trayRef.setToolTip('ModelForge - Update Available');
  } else {
    // Use normal icon
    if (isDev) {
